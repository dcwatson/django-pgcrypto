...
class CryptoFunc(Func):
    ...
    def get_params(self):
        cipher_name = self.params['cipher_name'] or getattr(
            self.field, 'cipher_name', getattr(settings, "PGCRYPTO_DEFAULT_CIPHER", "aes")
        )
        cipher_key = self.params['cipher_key'] or getattr(
            self.field, 'cipher_key', getattr(settings, "PGCRYPTO_DEFAULT_KEY", settings.SECRET_KEY)
        )
        charset = self.params['charset'] or getattr(self.field, 'charset', getattr(self.field, 'charset', "utf-8"))
